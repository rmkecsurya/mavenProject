...
public class UIAutomation extends BaseAutomation {
	 ...
	 */
	public Element get(final ControlType controlType,
			final String title,
	...
	public boolean isExists(final ControlType controlType,
			final String title,
			final int treeScopeConstant,
			final int numberOfRetries)
					throws AutomationException {
	...
	public Element wait(final ControlType controlType,
			final String name,
			final int timeInSecond)
					throws AutomationException {
		Element foundElement = null;

		int treeScopeConstant = TreeScope.DESCENDANTS;
		// And Condition
		PointerByReference pAndCondition = this.createAndCondition(
				this.createNamePropertyCondition(name),
				this.createControlTypeCondition(controlType));

		for (int loop = 0; loop < timeInSecond; loop++) {
	...
	public boolean isDisplayed(final ControlType controlType, final String name)
	{
		Element element = null;
		boolean status = false;
		try {
			element = wait(controlType, name, 5);
			status = !element.offScreen();
	...
	public Window getWindow(final String title)
			throws AutomationException {
//		FIND_DESKTOP_ATTEMPTS
		return getWindow(title, 5);
	}
