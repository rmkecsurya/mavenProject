...
public class Window
     ...
     * @throws AutomationException Something has gone wrong.
     */
    public MainMenu getMenu(String parent) throws AutomationException {
        return (
                new MainMenu(
                        new ElementBuilder(
                                this.getElementByControlType(
                                        parent, ControlType.Menu)).parent(
                                                this.getElement())));
    }
    
    
    public boolean isMenuEnable(String name)
	{
		try {
			getMenu(name);
			return true;
    ...
    public Element get(final ControlType controlType,
        ...
        for (int loop = 0; loop < numberOfRetries; loop++) {
            ...
            try {
            	foundElement = UIAutomation.getInstance().getRootElement().findFirst(new TreeScope(treeScopeConstant), pAndCondition);
            } catch (AutomationException ex) {
            }
